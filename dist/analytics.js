...
export const createAnalytics = (config) => {
    ...
    async function sendData(eventType, eventData, customData, priority = 'high') {
        ...
        return new Promise((resolve) => {
            state.batchTimeout = setTimeout(async () => {
                try {
                    ...
                    const hasHighPriorityEvents = state.eventQueue.some((queuedEvent) => queuedEvent.priority === 'high');
    ...
    function createEventPayload(eventType, eventData, customData) {
    ...
    const eventTracker = createEventTracker(sendData, state.pageLoadTime, {
    ...
    return {
        ...
        trackCustomEvent: (eventType, properties, customData) => {
            if (!shouldTrack())
                return;
            void sendData(eventType, properties, customData);
        },
        destroy: async () => {
            if (state.batchTimeout) {
                clearTimeout(state.batchTimeout);
